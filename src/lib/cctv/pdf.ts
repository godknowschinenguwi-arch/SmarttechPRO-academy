// SmartTech CCTV — branded proposal PDF generator (A4 portrait), styled to
// match the solar/fence proposal generators' brand system.
...
export interface CctvProposalData {
  design: CctvDesignResult;
  config: CctvConfig;
  appUrl: string;
}

export async function generateCctvProposalPdf({ design, config, appUrl }: CctvProposalData): Promise<Uint8Array> {
  ...
  text(p1, 'SMARTTECH CCTV', margin, H - 60, helvBold, 22, PAPER);
  ...
  const cards: [string, string][] = [
    ['Cameras', String(design.cameraCount)],
    ['Storage', `${design.totalStorageTb.toFixed(1)} TB`],
    ['NVR', `${design.nvr.channels}CH`],
  ...
  const engRows: [string, string][] = [
    ...
    ['Cable required', `${design.cableSpoolsNeeded} × ${design.cable.spoolLengthM}m spool`],
  ];
  engRows.forEach(([k, v]) => {
  ...
  const verifyUrl = `${appUrl}/cctv-calculator`;
  ...
  text(p1, 'lens field-of-view/lighting requirements, and local privacy/surveillance regulations.', margin, 57, helv, 7.5, FAINT);
  text(p1, 'SmartTech Academy — smarttech.academy', margin, 40, helvBold, 8.5, INK);

  pdf.setTitle(`SmartTech CCTV Proposal — ${config.clientName || 'Design'}`);
  pdf.setAuthor('SmartTech Academy');
  pdf.setSubject('CCTV system design proposal');
