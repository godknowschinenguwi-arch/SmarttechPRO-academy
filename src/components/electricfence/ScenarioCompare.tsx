...
import { computeFenceDesign, DEFAULT_FENCE_CATALOG } from '@/lib/electricfence/engine';
import { Price } from '@/components/CurrencyProvider';
import type { FenceScenario, FenceZone, FenceConfig, FenceCatalog } from '@/lib/electricfence/types';
...
export default function ScenarioCompare({
  ...
  scenarios: FenceScenario[];
  loading?: boolean;
  error?: string;
  signedIn?: boolean;
  currentZones: FenceZone[];
  currentConfig: FenceConfig;
  catalog?: FenceCatalog;
  onSave: (name: string) => void | Promise<void>;
  onLoad: (scenario: FenceScenario) => void;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="card flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
        <input
          ...
          placeholder="Name this design (e.g. Warehouse perimeter)"
          value={name}
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-sm text-ink-faint">
          Save designs above to compare different perimeters or fence specs side by side.
        </div>
      ) : (
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b border-surface-line bg-surface-soft text-left text-[11px] font-bold uppercase tracking-wide text-ink-faint">
                ...
                <th className="px-4 py-2.5">Perimeter</th>
                <th className="px-4 py-2.5">Energizer</th>
            ...
            <tbody className="divide-y divide-surface-line">
              {scenarios.map((s) => {
                const d = computeFenceDesign(s.zones, s.config, catalog);
                return (
                  <tr key={s.id}>
                    <td className="px-4 py-3 font-semibold text-ink">{s.name}</td>
                    <td className="px-4 py-3">{(d.totalPerimeterM / 1000).toFixed(2)} km</td>
                    <td className="px-4 py-3">{d.energizerCount} × {d.energizer.joulesOutput}J</td>
                    <td className="px-4 py-3">{d.postCount}</td>
