'use client';
import type { FenceAccessoryCategory, CatalogFenceAccessory, SelectedAccessory } from '@/lib/electricfence/types';

const CATEGORY_ORDER: FenceAccessoryCategory[] = ['COMPRESSION_SPRING', 'HOOK', 'COPPER_FERRULE', 'FENCE_LIGHT', 'LIGHTNING_DIVERTER', 'OTHER'];
const CATEGORY_LABELS: Record<FenceAccessoryCategory, string> = {
  COMPRESSION_SPRING: 'Compression springs',
  HOOK: 'Hooks',
  COPPER_FERRULE: 'Copper ferrules',
  FENCE_LIGHT: 'Fence lights',
  LIGHTNING_DIVERTER: 'Lightning diverters',
    ...
    <div className="flex flex-col gap-6">
      ...
      {totalSelected === 0 && (
        <p className="card p-6 text-center text-sm text-ink-faint">
          No accessories selected yet — add compression springs, hooks, copper ferrules, fence lights or lightning
          diverters above. They&apos;ll be added to the bill of materials.
        </p>
      )}
    </div>
  );
