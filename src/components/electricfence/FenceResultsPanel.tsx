  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <SummaryStat label="Perimeter" value={`${(design.totalPerimeterM / 1000).toFixed(2)} km`} sub={`${design.totalWireLengthM.toLocaleString(undefined, { maximumFractionDigits: 0 })}m wire`} />
        <SummaryStat label="Energizer" value={`${design.energizer.joulesOutput}J`} sub={`${design.energizerCount} × ${design.energizer.model}`} />
        <SummaryStat label="Posts" value={String(design.postCount)} sub={`${design.insulatorCount} insulators`} />
      ...
      <div className="card p-4">
        ...
        <p className="mt-3 text-[11px] leading-relaxed text-ink-faint">
          ...
          conditions, moderate vegetation and correctly maintained insulation — final design must be verified by a
          qualified installer against a site survey and local electrical/security regulations.
        </p>
      </div>
    </div>
  );
