...
import { buildAssistantSystemPrompt } from '@/lib/cctv/assistant';
...
export async function POST(req: NextRequest) {
  ...
  const limited = rateLimit(`cctv-assistant:${clientIp(req)}`, 15, 60 * 60_000);
  if (!limited.ok) {
    return NextResponse.json(
  ...
  const system = buildAssistantSystemPrompt(points, config, design);
  ...
  if (data.stop_reason === 'refusal') {
    return NextResponse.json({
      reply: "I can't help with that request. Try asking about your CCTV system's sizing, equipment, or costs.",
    });
  }
