...
import { buildAssistantSystemPrompt } from '@/lib/wiring/assistant';
...
export async function POST(req: NextRequest) {
  ...
  const limited = rateLimit(`wiring-assistant:${clientIp(req)}`, 15, 60 * 60_000);
  if (!limited.ok) {
    return NextResponse.json(
  ...
  const system = buildAssistantSystemPrompt(circuits, config, design);
  ...
  if (data.stop_reason === 'refusal') {
    return NextResponse.json({
      reply: "I can't help with that request. Try asking about your wiring design's sizing, equipment, or costs.",
    });
  }
