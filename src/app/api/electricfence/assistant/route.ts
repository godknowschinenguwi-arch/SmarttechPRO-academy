...
import { buildAssistantSystemPrompt } from '@/lib/electricfence/assistant';
...
export async function POST(req: NextRequest) {
  ...
  const limited = rateLimit(`fence-assistant:${clientIp(req)}`, 15, 60 * 60_000);
  if (!limited.ok) {
    return NextResponse.json(
  ...
  const system = buildAssistantSystemPrompt(zones, config, design);
  ...
  if (data.stop_reason === 'refusal') {
    return NextResponse.json({
      reply: "I can't help with that request. Try asking about your fence's sizing, equipment, or costs.",
    });
  }
