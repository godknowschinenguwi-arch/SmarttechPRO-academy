...
import { cameraStore, nvrStore, hddStore, cableStore, poeSwitchStore } from '@/lib/cctv/catalogDb';
...
export default async function CctvCatalogAdminPage() {
  ...
  const [cameras, nvrs, hdds, cables, poeSwitches] = await Promise.all([
    ...
    <div className="container-x space-y-6 py-10">
      <div>
        ...
        <p className="mt-1 text-ink-faint">
          Add, edit, deactivate or remove cameras, NVR/DVRs, storage, cabling and PoE switches. Changes apply
          immediately to the live CCTV Calculator — no redeploy needed.
        </p>
      </div>

      <CctvCatalogManager cameras={cameras} nvrs={nvrs} hdds={hdds} cables={cables} poeSwitches={poeSwitches} />
    </div>
  );
}
